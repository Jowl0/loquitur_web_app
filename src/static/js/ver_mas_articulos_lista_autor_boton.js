document.addEventListener('DOMContentLoaded', function() {
    const btnVerMas = document.getElementById('btn-ver-mas-articulos-lista-autor');
    const btnContainer = document.getElementById('btn-container');
    let contador = 1;

    if (btnVerMas) {
        btnVerMas.addEventListener('click', function () {
            const idAutor = this.getAttribute('data-autor');
            const urlApi = '/api/ver_mas/articulos/autor';
            const queryParams = `?contador=${contador}&id_autor=${idAutor}`;
                        ...
                        if (alertContainer) {
                            alertContainer.innerHTML = `
                                <div class="alert alert-dismissible alert-light border shadow-sm text-center fade show mx-auto" style="max-width: 500px;" role="alert">
                                    ...
                                    <strong>¡Todo leído!</strong> No hay más artículos publicados por este autor.
                                </div>`;
                        }
                    }
                })
